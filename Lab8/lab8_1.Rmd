...
With BLASTn we can take a sequence we have aquired (perhaps through sequencing) and search GenBank for similar sequences. Go to the NCBI BLAST link above, then click "Nucleotide BLAST". In your data folder is a file called "Ummidia.fasta". Open this file in your text editor. This file contains 28 COI sequences in what is called fasta format. Copy one of these sequences, beginning with the carrot through the end of the sequence (before the next carrot) and paste it into the Query Sequence Box. We will now do a search with the deafult settings.
...
We can use ape to search the GenBank database just like we did using the web portal. We can use the function read.GenBank() to read nucleotide sequences into R using accession numbers as arguments. Take a look at the help page and usage options for this function.
```{r}
?read.GenBank()
```
...
That's a little better, now we can see the nucleotides that make up the sequence stored as a vector. Let's try fetching multiple sequences at once and storing them as a DNAbin object. This is a list of accession numbers for RAG1 sequences for a group of lizards. We're going to store this list as a vector...
```{r}
...
...and then add our sequences to it. Looking at this new object again we see some basic information on the sequences we've pulled from the database.
```{r}
...
Let's explore this data just a little bit. We can use functions like str(), attributes(), names(), and attr() to get different kind of information to include our accession numbers, the contents of the data, and a list of the included species. Try each of these functions and compare the output.
```{r}
...
This data would probably be easire to read is we made a vector that matched up our accession numbers with our species. Let's do that! We'll add the name of the gene we're looking at for good measure. This is good practice, especially if you might want to add other sequences later.
```{r}
lizards_sequences_GenBank_IDs <- paste(attr(lizards_sequences, "species"), names(lizards_sequences), sep ="_RAG1_")
```


Generally when we work with sequence data we want it in a specific format such as a .fasta or .phylip because the software and packages we use to analyze sequence data need to have the data in a predictable format. We can use the write.dna function to write our sequences into a .fasta format.
```{r}
...
Let's look for our lizards by their species name Basiliscus basiliscus using entrez_search(). We can tell entrez we want to search the nucleotide database (nuccore) for Basiliscus basiliscus which is an [Organism]. We'll also set the maximum returned hits to 40 (retmax=40).
```{r}
...
Again, this will be more useful for us stored as a .fasta file. We can use the write() function to do this. Look very closely at those sequences. It looks like a big mess of text but there are "new line delimiters" in that text and we need to tell R what they are so it knows where the line endings are. In this case the new line delimiter is "\n", so we will write the data to a .fast file like this:
```{r}
...
1. Create your own search of the NCBI nucleotide database using entrez_search(). Limit your returned hits to 10.
```{r}

```


2. Use entrex_fetch() to return your database hits as a character vector.
```{r}

```


3. Now store your own pulled sequence data as a .fasta file using the write() function. Remember thos line endings!
```{r}
