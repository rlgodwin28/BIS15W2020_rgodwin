...
One variable frequently reported on our sequence data in publications is the "GC content" or the fraction of the sequence that is G's and C's.
```{r}
GC(dengueseq)
```

## Practice
1. Have them download another sequence and perform the same calculations.

Next, we are going to look at COI sequence data from gophers and their assocaited chewing lice. We'll start by using the `read.alignment()` function to read in the .fasta alignment files for each group and have a look at them. The difference between sequences and an alignment is that the positions of the nucleotides have been aligned such that they are homologous.
```{r}
...
We know each of the louse species is associated with a specific gopher host. Perhaps it would be cool to create phylogenetic trees of both groups and see if there is congruence between host and parasite evolution. Right? I thought so!  

Step 1: Infer the trees! For our purposes we will use a  neighbor-joining algorithm under the Juke and Cantor's correction for multiple substitions (don't stress too much about that last word jumble, let's go!)
```{r}
...
All right, let's step it up a notch. We may not be super duper programmers, but there are super duper programmers and they write cool scripts we can use. One such script is in your data folder, and it's called "essai.r" This script creates the same tree diagram but in R's "graphical device." The nice thing about using a script is that, similar to a markdown file, it is relatively simple to edit and also to share with others. 
```{r}
source("essai.r")
```
